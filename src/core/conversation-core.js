...
function modelContextWindow(history, currentMessage, maxChars = MAX_MODEL_CONTEXT_CHARS) {
  ...
  for (let index = prior.length - 1; index >= 0; index -= 1) {
    ...
    if (used + candidate.content.length > budget) break;
...
module.exports = { alignComparisonQuery, comparisonTargetForQuery, directlyIdentifiesComparisonTarget, filterComparisonOutcome, mergeScopedOutcomes, modelContextWindow, preserveContextQualifierQuery, preserveRequestedPlatformQuery, unavailableSearchOutcome, ConversationCore, MAX_MESSAGE_LENGTH, MAX_MODEL_CONTEXT_CHARS };
