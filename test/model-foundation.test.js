...
test('model foundation evaluator records local passes and keeps semantic cases NOT VERIFIED', () => {
  ...
  assert.deepEqual(report.counts, { PASS: 21, FAIL: 0, 'NOT VERIFIED': 4 });
  ...
  assert.equal(report.results.find(item => item.id === 'correction_handling').status, 'PASS');
  assert.equal(report.results.find(item => item.id === 'long_context').status, 'PASS');
