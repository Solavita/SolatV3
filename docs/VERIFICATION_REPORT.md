...
| Deterministic stability and performance budget | PASS | `npm.cmd run benchmark:model-foundation` | 2,000 deterministic evaluations; zero mismatches; p95 below 100 ms; heap growth below 64 MiB | `scripts/benchmark-model-foundation-local.js`; `test/model-foundation-performance.test.js` | Measures local router/evaluator contracts only |
