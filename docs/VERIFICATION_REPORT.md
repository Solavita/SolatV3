...
| criterion | status | exact command/action | exact input/output | files/evidence | limitation |
|---|---|---|---|---|---|
| Attached text/CSV assets can reach the review-only commerce intake endpoint | PASS | Added owner/session-scoped asset resolution and multipart `commerce_intake_file`; ran `npm.cmd test` and `npm.cmd run check` | `101/101` tests passed; multipart request carries the stored asset bytes and `X-User-ID`; no persistent record is created by the intake endpoint | `src/core/commerce-client.js`; `src/core/conversation-core.js`; `src/main.js`; `test/core.test.js` | Requires the local backend intake route; only supported UTF-8 text/CSV types are accepted by the backend |
| Payment-slip image intake remains review-only and fail-closed | PASS | Exposed `commerce_payment_slip_intake` through the same asset bridge | Action requires an `asset_id`, returns a draft from the backend OCR boundary, and never marks an order paid; missing resolver/asset is reported as unavailable | `src/core/commerce-client.js`; `src/core/conversation-core.js`; `app/api/routes/commerce.py` | OCR/payment verifier is not configured, so real slip verification remains `NOT VERIFIED` |
