...
async function main() {
  ...
  if (!config.apiKey) throw Object.assign(new Error('A configured model provider is required.'), { code: 'provider_not_configured' });
  ...
  try {
    ...
    process.stdout.write(`${JSON.stringify({ schema_version: 'solat.live-youtube-planner-smoke.v1', status: 'PASS', provider: config.provider, model: config.model, planned_tool: action.tool, query: action.arguments.query, approval_required: true, computer_mutations_before_approval: 0 }, null, 2)}\n`);
  } finally {
